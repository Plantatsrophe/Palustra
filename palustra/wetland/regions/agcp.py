...
# AGCP Regional Hydrology Tiers
AGCP_HYDROLOGY_TIERS: Dict[str, str] = {
    ...
    "B4": "Primary",
    "B5": "Primary",
    "B6": "Primary",    # Surface Soil Cracks: PRIMARY in AGCP
    "B7": "Primary",
    "B8": "Primary",
    "B9": "Primary",    # Water-Stained Leaves: PRIMARY in AGCP
    "B10": "Secondary",
    "B11": "Primary",
    "B12": "Secondary",
    "B13": "Primary",
    ...
    "D6": "Not Recognized",  # Sphagnum Moss: NOT RECOGNIZED in AGCP
    "D7": "Not Recognized",  # Frost-Heave Hummocks: NOT RECOGNIZED in AGCP
}

# Approved NRCS Hydric Soil Indicators in AGCP (v8.2)
AGCP_APPROVED_SOIL_INDICATORS: Set[str] = {
    # All Soils
...
# Curated regional indicator ratings for AGCP (including key species that shift regionally)
AGCP_TAXON_INDICATOR_RATINGS: Dict[str, str] = {
    # Shifting taxa
    "QUERCUS NIGRA": "FACW",
    "QUNI": "FACW",
    "ROBINIA PSEUDOACACIA": "UPL",
    "ROPS": "UPL",
    ...
    "RHYNCHOSPORA CHALAROCEPHALA": "OBL",
...
class AGCPPolicy(RegionalSupplementPolicy):
    """Atlantic and Gulf Coastal Plain Regional Supplement (AGCP) Strategy."""

    def __init__(self, custom_indicators: Optional[Dict[str, str]] = None) -> None:
        self._indicators: Dict[str, str] = dict(AGCP_TAXON_INDICATOR_RATINGS)
    ...
    def region_code(self) -> RegionalSupplementEnum:
        return RegionalSupplementEnum.AGCP

    @property
    def region_name(self) -> str:
        return "Atlantic and Gulf Coastal Plain Regional Supplement (Version 2.0)"

    def get_indicator_status(self, taxon_id: str, default_status: str = "NL") -> str:
        """Resolve regional indicator status under the AGCP NWPL list."""
        if not taxon_id:
    ...
    def get_stratum_definitions(self) -> Dict[str, StratumCriteria]:
        """Return 4-stratum criteria per AGCP Regional Supplement."""
    ...
    def get_hydrology_tier(self, indicator_code: str) -> str:
        """Return regulatory tier for an indicator code in AGCP."""
        clean = indicator_code.strip().upper()
        return AGCP_HYDROLOGY_TIERS.get(clean, "Not Recognized")
    ...
    def validate_hydrology_indicators(
        ...
        """Evaluate wetland hydrology compliance for AGCP.

        Criteria: >= 1 confirmed Primary OR >= 2 confirmed Secondary indicators.
        Filters inputs to indicators that are recognized in AGCP with their designated tier.
        """
    ...
    def approved_soil_indicators(self) -> Set[str]:
        return set(AGCP_APPROVED_SOIL_INDICATORS)

    def validate_hydric_soil_indicators(self, indicators: List[str]) -> bool:
        """Evaluate if at least one approved hydric soil indicator is confirmed for AGCP."""
